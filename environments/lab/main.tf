    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${module.eks_fargate_karpenter.oidc_provider}:sub" = "system:serviceaccount:kube-system:aws-load-balancer-controller"
            "${module.eks_fargate_karpenter.oidc_provider}:aud" = "sts.amazonaws.com"
          }
        }
      }
    ]
